...
Command.prototype.run = function (data, flagMap, done) {
  ...
  async.series({
    ...
    handler: function (done) {
      ...
      var isAsync = command.isAsync();
      
      // async
      if (isAsync) {
        data = data.concat(done); // add callback to arguments list
      }
...
Command.prototype.runBefores = function (data, flags, done) {
  ...
  this.internals.befores.run(data, flags, done);
...
Command.prototype.runAfters = function (data, flags, done) {
  ...
  if (this.isAsync()) {
    this.internals.afters.run(data, flags, done);
  }
  else {
    this.internals.afters.run(data, flags);
    done();
  }
...
Command.prototype.async = function (val) {
  ...
  this.internals.async = (val !== false) ? true: false; // Is true if no value
  
  // TODO: test these
  
  this.internals.befores.internals.async = true;
  this.internals.afters.internals.async = true;
