...
Command.prototype.runFlags = function (flagMap, done) {
  ...
  // TODO: flag should run itself. COmmand shouldn't know if it is async of not.
  // Command should assume that flags are async by defult and pass in a callback
  /*
    this.internals.flags.run(function () {
      // done running
    });
  */
  
  // Set all flags as async or not
  // this.internals.flags.async(isAsync);
  
  // if (isAsync) {
    
  //   // Find flags to run
  //   var runners = _(flagMap)
  //     .keys()
  //     .map(function (name) {
        
  //       var flag = self.internals.flags.findByName(name);
  ...
  //       else {
  //         return {
  //           flag: self.internals.flags.findByName(name),
  //           data: flagMap[name]
  //         };
  //       }
  //     })
  ...
  //   // Run flags
  //   async.each(runners, function (runner, flagDone) {
      
  //     runner.flag.runOnce(runner.data, flagDone);
  //   }, done);
  // }
