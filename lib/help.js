...
var help = function (cli) {
  ...
  var helpCommand = cli.command('help')
    ...
    .handler(function () {
      printIntro();
      printUsage('  ' + programName() + ' <command> <parameters> <options>');
      printCommands();
      printOptions();
    });
  ...
  helpCommand.task('detail')
    .handler(function (cmd, done) {
      ...
      _.each(command._aliases, function (alias, idx) {
        usageStr += '  ' + programName() + ' ' + alias;
        if (idx < command._aliases.length - 1) usageStr += '\n  ';
      });
