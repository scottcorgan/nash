...
function parseArgv (processArgv, rawFlags) {
  ...
  return minimist(processArgv ? processArgv.slice(2) : [], argvOptions);
}
...
Cli.prototype.run = function (processArgv, runDone) {
  ...
  var self = this;
  var command = parseCommand(minimist(processArgv ? processArgv.slice(2) : []));
