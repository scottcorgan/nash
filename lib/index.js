...
// TODO: should this have an async mode as well?
Cli.prototype.defaultCommand = function (fn) {
  
  this.internals.defaultCommand = fn.bind(this);
  ...
  async.series({
    ...
    command: function (done) {
      ...
      self.internals.commands.run(command, function (err) {
        ...
        if (err && typeof self.internals.defaultCommand === 'function') {
          self.internals.defaultCommand(command.flags);
        }
