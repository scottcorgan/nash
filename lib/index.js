    ...
    command: function (done) {
      ...
      self.internals.commands.run(command, function (err) {
        ...
        if (err && self.internals.defaultCommand) {
          ...
          command.data = _.initial((self.argv._ || []).concat(command.data));
...
Cli.prototype.runBeforeAlls = function (data, flags, done) {
  ...
  this.internals.beforeAlls.run(data, flags, done);
...
Cli.prototype.runAfterAlls = function (data, flags, done) {
  ...
  this.internals.afterAlls.run(data, flags, done);
...
Cli.prototype.register = function (plugins, done) {
  ...
  async.eachSeries(plugins, function (plugin, doneRegistering) {
    ...
    plugin.register.register(cli, plugin.options || {}, doneRegistering);
  }, done);
  
  return this;
};

function parseArgv (processArgv, flags) {
