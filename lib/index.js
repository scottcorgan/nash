...
Cli.prototype.run = function (argv, runDone) {
  ...
  async.series({
    ...
    flags: function (done) {
      ...
      if (commandToRun) {
        commandFlags = commandToRun.internals.flags;
      }
...
Cli.prototype.runFlags = function (flagMap, done, _commandFlags_) {
  ...
  var flags = this.internals.flags;
  var flagOverrides = [];
  
  // TODO: test _commandFLags_
  // Command level flag overrides
  if (_commandFlags_) {
    flagOverrides = _.filter(_commandFlags_, function (f) {
      
      return f.shouldOverride();
    });
    
    _.each(flagOverrides, function (flagOverride) {
      
      var flagIndex = -1;
      var matchingFlag = _.find(flags, function (f, idx) {
        
        var matches = f.matchesName(flagOverride.name());
  ...
  _(flagMap).keys().each(function (flagName) {
    ...
    var flagsToRun = _.filter(flags, function (flag) {
