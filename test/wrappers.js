...
test('wrappers: adds multiple wrappers at a time', function (t) {
  ...
  t.deepEqual(wrap1.all(), [[fn1], [fn1, fn2]], 'adds multiple fns from array');
  t.deepEqual(wrap2.all(), [[fn1, fn2]]), 'adds multiple fns as arguments'
  t.end();
...
test('wrappers: runs all the wrapper fns', function (t) {
...
test('wrapprs: runs wrapper fns with given values passed into fns', function (t) {
...
test('wrappers: runs fns as async by passing in a callback as the last argument');
